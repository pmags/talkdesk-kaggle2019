...
# Class msts with multiseaonability based on eda
msts_data <- msts(select(data,-interval),seasonal.periods = c(96,96*7,96*31,96*31*6))
...
m <- add_seasonality(m, name="monthly", period = 30.5, fourier.order = 215)
m <- add_seasonality(m, name="daily", period = 1, fourier.order = 15)
m <- add_seasonality(m, name="weekly", period = 7, fourier.order = 215)
m <- add_seasonality(m, name="semester", period = 365.25/2, fourier.order = 15,prior.scale = 15)
...
# cv_calls <- cross_validation(m,initial = 67104/1.3, period = 96*31*12,horizon = 96*31)
# performance_metrics(cv_calls)
...
fit_duration <- add_seasonality(fit_duration, name="monthly", period = 30.5, fourier.order = 20)
fit_duration <- add_seasonality(fit_duration, name="daily", period = 1, fourier.order = 25)
fit_duration <- add_seasonality(fit_duration, name="weekly", period = 7, fourier.order = 20)
fit_duration <- add_seasonality(fit_duration, name="semester", period = 365.25/2, fourier.order = 20,prior.scale = 15)
...
mape_dur <- xts_dur %>% as.data.frame() %>% 
  mutate(t = abs(y-yhat)/y*100) %>%
  mutate(t = ifelse(is.infinite(t),0,t)) %>%  
  summarise(MAPE = mean(t,na.rm = TRUE))
...
agent_model <- lm(agent_headcount ~ poly(total_calls_duration, 8, raw = TRUE), data = xts_data)
...
write.csv(csv_result,file = "submissions/24-09-2019_0-25.csv",row.names = FALSE,quote = FALSE)
