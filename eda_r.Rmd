...
# Single features distributions

```{r}
sapply(select(data,-interval),FUN = function(x){sum(is.infinite(x))})
```

```{r}
sapply(select(data,-interval),FUN = function(x){sum(is.na(x))})
```


## Agent headcount

This is a time series evenly spaced with no missing data

```{r echo=FALSE}
# convert to time base

monthly_headcount <- to.monthly(as.xts(x = data$agent_headcount, order.by = as.POSIXct(strptime(data$interval,"%Y-%m-%d %H:%M:%S"), tz = "UTC")))

chart_Series(monthly_headcount,multi.col=TRUE, TA = NULL)

```

```{r}
ep1 <- endpoints(as.xts(x = data$agent_headcount, order.by = as.POSIXct(strptime(data$interval,"%Y-%m-%d %H:%M:%S"), tz = "UTC")),on="days") 

plot(period.apply(as.xts(x = data$agent_headcount, order.by = as.POSIXct(strptime(data$interval,"%Y-%m-%d %H:%M:%S"), tz = "UTC")),on="days",INDEX=ep1,FUN=mean),type = "l", xlab = "Headcount (nº agents)" , main = "Average daily agent headcount")
```



```{r echo=FALSE}
ep2 <- endpoints(as.xts(x = data$agent_headcount, order.by = as.POSIXct(strptime(data$interval,"%Y-%m-%d %H:%M:%S"), tz = "UTC")),on="months") 

plot(period.apply(as.xts(x = data$agent_headcount, order.by = as.POSIXct(strptime(data$interval,"%Y-%m-%d %H:%M:%S"), tz = "UTC")),on="months",INDEX=ep2,FUN=mean),type = "b", xlab = "Headcount (nº agents)" , main = "Average monthly agent headcount")
